    ...
    with jsonlines.open(dataset_path) as reader, jsonlines.open(output_file, mode='w', flush=True) as writer:
        for i, problem in enumerate(reader):
            ...
            if len(clusters.get_cluster_list()) == 1:
                result["llm_ground_truth"] = 0
            else:
                result, reason = specfix_accuracy_evaluator.classification(problem["requirement"])
                if result == "Yes":
                    result["llm_ground_truth"] = 1
                elif result == "No":
                    result["llm_ground_truth"] = 0
                else:
                    result["llm_ground_truth"] = -1
                result["llm_ground_truth_reason"] = reason
            writer.write(result)
